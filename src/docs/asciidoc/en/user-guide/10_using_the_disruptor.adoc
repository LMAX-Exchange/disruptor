...
WARNING: There is only one use-case where an actual lock is required and that is within the `BlockingWaitStrategy`.
...
In our case all we want to do is print the value out to the console.
...
We will need a source for these events.
...
CAUTION: It is also necessary to wrap publication in a try/finally block.
...
Therefore, it is recommended that either lambdas or the `EventTranslator` APIs be used.
...
----
<1> Construct the Disruptor with a `SingleProducerSequencer`
