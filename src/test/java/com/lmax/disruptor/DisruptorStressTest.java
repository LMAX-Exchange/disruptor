...
public class DisruptorStressTest {
    ...
    public void shouldHandleLotsOfThreads() throws Exception {
        Disruptor<TestEvent> disruptor = new Disruptor<>(TestEvent.FACTORY, 1 << 16, DaemonThreadFactory.INSTANCE, ProducerType.MULTI, new BusySpinWaitStrategy());
    ...
    private Publisher[] initialise(final Publisher[] publishers, final RingBuffer<TestEvent> buffer, final int messageCount, final CyclicBarrier barrier, final CountDownLatch latch) {
    ...
    private static class Publisher implements Runnable {
        ...
        Publisher(final RingBuffer<TestEvent> ringBuffer, final int iterations, final CyclicBarrier barrier, final CountDownLatch shutdownLatch) {
